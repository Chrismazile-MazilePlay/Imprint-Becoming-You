...
///
/// Used internally by `SpeechAnalysisService` for audio analysis during
/// voice calibration and speech capture. The resonance score is no longer
/// displayed to users (replaced by word-by-word highlighting), but the
/// calculator remains for audio level processing and calibration.
...
///
/// Used by `VoiceCalibrationService` and `SpeechAnalysisService` for
/// pitch detection during audio capture.
