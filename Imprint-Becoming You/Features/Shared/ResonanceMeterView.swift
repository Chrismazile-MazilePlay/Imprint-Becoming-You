    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: AppTheme.Spacing.xs) {
                ...
                // Quality badge
                if animatedScore > 0 {
                    QualityBadge(quality: ResonanceRating(score: animatedScore))
        ...
        .accessibilityValue(ResonanceRating(score: score).rawValue)
    }
    
    // MARK: - Computed Properties
    
    private var scoreColor: Color {
        let quality = ResonanceRating(score: animatedScore)
        switch quality {
        case .needsWork:
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.sm) {
            ...
            HStack {
                ...
                if let best = scores.map(\.finalScore).max() {
...
struct ScoreBar: View {
    ...
    private var barColor: Color {
        let quality = ResonanceRating(score: score)
        switch quality {
        case .needsWork:
            return AppColors.resonanceNeedsWork
        case .fair:
